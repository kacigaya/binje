...
export default function RottenTomatoesRating({ imdbId }: { imdbId: string | null | undefined }) {
  const [result, setResult] = useState<{ imdbId: string; score: number | null } | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`/api/ratings/rt?imdbId=${encodeURIComponent(imdbId)}`, { signal: controller.signal })
  ...
  return (
    <div className="flex items-center gap-1.5 font-semibold text-accent-red">
      <Image src="/rotten-tomatoes.svg" alt="" width={16} height={16} aria-hidden="true" className="size-4 shrink-0" />
