...
import { getMovieCredits, getMovieDetails, getMovieImages, getSimilarMovies, getTVCredits, getTVDetails, getTVImages, getSimilarTV, pickLogo } from "@/lib/tmdb";
...
export async function GET(request: NextRequest, context: { params: Promise<{ type: string; id: string }> }) {
  try {
    ...
    if (type === "movie") {
      ...
      return mobileJson(serializeMovieDetails(detail, credits, related, score, images ? pickLogo(images.logos, lang) : null));
    }
    ...
    return mobileJson(serializeTVDetails(detail, credits, related, score, images ? pickLogo(images.logos, lang) : null));
