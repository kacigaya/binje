// Cloudflare Worker: vidfast stream resolver.
// Only job: hit vidfast.pro + enc-dec.app to get the m3u8 url. The Worker's
// Cloudflare egress gets past vidfast.pro's Cloudflare block (Netlify/AWS IPs
// are blocked there). Segment proxying stays on Netlify /api/hls — the stream
// CDN blocks the Worker's IP but allows Netlify's server-side (no Origin) fetch.
...
async function resolveStream(type, id, season, episode) {
  ...
  for (const server of serversDec.result ?? []) {
    ...
    const dec = await fetch(`${ENC_API}/dec-vidfast`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ text: streamEnc }),
    }).then((r) => r.json());
    if (dec.status === 200 && dec.result?.url) {
